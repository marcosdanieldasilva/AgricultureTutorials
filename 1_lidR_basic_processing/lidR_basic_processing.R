...
# Ricardo Dal'Agnol da Silva - 06 October 2020
# R version 4.0.2
...
## we will compare the two methods in the next step

# filter and visualize only the ground points
gnd = filter_ground(las_ground_pmf)
...
# classify ground again with the method potentially better and set of parameters that work best
ws = seq(3, 21, 3)
th = seq(0.1, 1.5, length.out = length(ws))
...
# First method: point2raster, simply assign the highest elevation within a grid (e.g. 1 x 1 m)
...
# Third method: pit free method
## Series of sequential height thresholds where Delaunay triangulations are applied to first returns
chm = grid_canopy(nlas, res = 0.5, pitfree(subcircle = 0.15, thresholds = c(0, 10, 20), max_edge = c(8, 1.5)))
plot(chm, col = hgt_col)
writeRaster(chm, filename = "chm_pitfree.tif", overwrite=T)


## Load all the CHM files in the QGIS and compare them visually. What are the differences of each one? Which one is 'better'?



# 6) Extracting LiDAR Metrics -----------------------------------------------------------------

# calc the CHM (the one you prefer from the last section)
chm = grid_canopy(nlas, res = 0.5, algorithm = p2r(subcircle = 0.15, na.fill = tin()))
plot(chm, col = hgt_col)
...
# 1) Point cloud Reading and Plotting: you can see the data frames and headers inside the LAS files with a few commands, the display tools are very easy to use
# 2) Ground classification: we saw two methods to filter ground points here, but it seems the PMF method did a better result for our area, at least without playing around with the parameters
# 3) Creating a DTM: it is easy to generate a DTM surface, there are more than 3 methods to do this, the simplest being the TIN method, TIN method is based on the Delaunay Triangulation, it needs NO parameters (this is very good), however it cannot extrapolate outside of the bounding box and its weaker at the edges
# 4) Height Normalization: forest height is now normalized and you can see the differences in tree height
# 5) Creating a CHM: which CHM is better depends on the purpose. P2R with disc is simple but leaves lots of empty cells, triangulation smooths a lot, Pitfree is less smoothed than delaunay and fills lots of holes from the P2R with disc
# 6) LiDAR Metrics: lots of metrics, their usefulness depend on the application, e.g. they can be very useful for biomass estimates
